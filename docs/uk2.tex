...
Let us define $q$ as the "quasi-velocity" vector:
\begin{align}
    q_i &=
    \begin{bmatrix}
        v_{i/0}^0 \\
...
Which is related to the derivative of the state $\dot{x}$ by the following relation.

\begin{align}
    \dot{x}_i &=
    \begin{bmatrix}
        I_3 & 0_{3 \times 3} \\
        0_{3 \times 3} & \frac{1}{4} B(\sigma_0^i)
    \end{bmatrix} q_i = H_iq_i
\end{align}
\begin{align}
    \dot{x} &=
    \begin{bmatrix}
...
Where $B(\sigma)$ is defined as:

\begin{align}
    B(\sigma) &= (1 - \sigma^T\sigma)I + 2\tilde{\sigma} + 2\sigma\sigma^T
\end{align}

Using $q$, we can leave our constraint equations in terms of $q$ to get $\dot{q}$, and
then integrate $x$ by plugging in $Hq$.  With this, we can get $A$ and $b$ for our UK
constraint equation

\begin{align}
    A =
    \begin{bmatrix}
        I_3 & -C((\sigma_0^1)^{-1})\tilde{r}_{j/1}^1 & -I_3 &
        C((\sigma_0^2)^{-1})\tilde{r}_{j/2}^2 \\
        0_{3\times3} & \tilde{v}C((\sigma_0^1)^{-1})\tilde{j}_1 &
        0_{3\times3} & -\tilde{u}C((\sigma_0^2)^{-1})\tilde{j}_2
    \end{bmatrix}
\end{align}
\begin{align}
    \begin{split}
    ...
    \end{split} \\
    \begin{split}
        b_\sigma &= \tilde{v}C((\sigma_0^1)^{-1})(\tilde{\omega}_{1/0}^1)^2j_1 -
...
The equations of motion for this rigidbody can be broken into two parts, the
translational equations of motion, and the rotational equations of motion.  The
translational equations are pretty simple.
...
\begin{align}
    I_i^i\dot{\omega} &= T - \omega\times I_i^i\omega
\end{align}
